    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Your environmental impact with Tech4Green — carbon, water, diesel and treatment savings.</p>
        </div>
        {months.length > 0 && (
...
function EsgMonth({ row }: { row: EsgResultCustomerRow }) {
  ...
  const cards: { key: string; label: string; value: number | null; unit: string; icon: LucideIcon; tone: string }[] = [
    { key: 'co2e_saved_kg', label: 'CO₂e saved', value: row.co2e_saved_kg, unit: 'kg', icon: Leaf, tone: 'emerald' },
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <div className="flex items-center justify-between"><p className="text-xs font-medium text-gray-500 uppercase tracking-wider">CO₂e saved</p><Badge b={b.co2e_saved_kg} /></div>
